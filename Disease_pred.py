...
diabetes_model = pd.read_pickle(open("trained_models/diabetes_model.sav",'rb'))
heart_model = pd.read_pickle(open("trained_models/heart_model.sav",'rb'))
...
def diabetes_pred(input_data):
    ...
    inp_np_arr = np.asarray(input_data,dtype=float)
...
def heart_disease_pred(input_data):
    ...
    inp_np_arr = np.asarray(input_data,dtype=float)
    inp_data_reshaped = inp_np_arr.reshape(1,-1)
    pred1 = heart_model.predict(inp_data_reshaped)
    if pred1[0] == 0:
...
if (selected=='Heart Disease prediction'):
    ...
    if st.button('Heart disease test result'):
        diagnosis = heart_disease_pred([aged,sex,cp,trestbps,chol,fbs,resrecg,thalach,exang,oldpeak,slope,ca,thal])
